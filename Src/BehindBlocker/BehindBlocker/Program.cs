...
namespace BehindBlocker
{
    internal class Program
    {
        ...
        private static bool checking, checkingdomain, checkingscaledip;
        ...
        static void Main(string[] args)
        {
            ...
            using (StreamReader filedomain = new StreamReader("domain.txt"))
            {
                while (filedomain.Peek() >= 0)
                {
                    domains.Add(filedomain.ReadLine());
                }
            }
            Task.Run(() => Start());
            Console.ReadKey();
                    ...
                    try
                    {
                        ...
                        if (RemoteAdrr != "::1" & RemoteAdrr != "127.0.0.1" & RemoteAdrr != "0.0.0.0")
                        {
                            ...
                            if (!checking)
                            {
                                ...
                                hostname = Dns.GetHostEntry(Addr.ToString()).HostName;
                                ...
                                if (!checkingdomain | !IsServer(remoteip))
                                {
                                    Console.WriteLine(hostname + ", " + RemoteAdrr + ", Blocked");
